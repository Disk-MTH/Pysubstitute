...
def proceed(entry_old_string, entry_new_string, entry_target_folder):
    ...
    if Utils.check_empty_entry(entries_to_check={"entry_old_string": old_string,
        ...
        if Utils.change_string(target_folder, old_string, new_string):         # The function is called 3 times because I don't
            if Utils.change_string(target_folder, old_string, new_string):     # know why but some files are not changed correctly
                if Utils.change_string(target_folder, old_string, new_string): # (except with 3 passes) and as it's a small project
